    ...
    (for {
      ...
      case (userIntegrations, users, groups, integrationGroups, topic, integrationTopic, topics) =>
        Ok(Json.toJson(JsObject(Seq(
          ...
          "groups" -> groups,
          "topic" -> (topic match {
            case Some(value) => getTopicJsValue(value, Group(value.groupId, ""), new User(value.userId, "", "", None, None, false)) // TODO: Fix me
            case None => JsNull
          }),
          "topics" -> topics,
