  ...
  def logout(id: String, discardUserCookie: Boolean) = Action.async { implicit request =>
    integrations.toSeq.find(_.id == id) match {
      case Some(integration) =>
        var redirect = Redirect(controllers.routes.Application.index("", 1, None, None, None, None, None, None, None, None, None, None).absoluteURL(RequestUtils.secure))
            ...
            usersDAO.findByLogin(login).flatMap {
              case Some(user) =>
                integrationTokensDAO.find(user.id, integration.id).flatMap {
                  ...
                  case _ => {
                    // Already disabled
                    Future.successful(Redirect(controllers.routes.Application.index("", 1, None, None, None, None, None, None, None, None, None, None).absoluteURL(RequestUtils.secure)))
                  }
                }
                ...
                integrationTokensDAO.find(user.id, integration.id).flatMap {
                  case Some(token) =>
                    integrationTokensDAO.merge(IntegrationToken(token.userId, token.integrationId, token.token, true)).flatMap { _ =>
                       ...
                       Future.successful(Redirect(redirectUrl.getOrElse(controllers.routes.Application.index("", 1, None, None, None, None, None, None, None, None, None, None).absoluteURL(RequestUtils.secure))))
                    }
                  case _ =>
                ...
                request.cookies.get("user") match {
                  case Some(cookie) =>
                    ...
                    (for {
                      ...
                      Redirect(redirectUrl.getOrElse(controllers.routes.Application.index("", 1, None, None, None, None, None, None, None, None, None, None).absoluteURL(RequestUtils.secure)))
                    ...
                    integrationUsersDAO.findByIntegrationUserId(integrationUserId, integrationId).flatMap {
                      case Some(integrationUser) if integrationUser.userId.nonEmpty =>
                        ...
                        (for {
                          ...
                          Redirect(redirectUrl.getOrElse(controllers.routes.Application.index("", 1, None, None, None, None, None, None, None, None, None, None).absoluteURL(RequestUtils.secure))).withCookies(
